...
const { ValidationError, SymbolError, SessionError } = require('../errors');
const { isDebugEnabled } = require('../config');

const studyConstructor = require('./study');

// Constants
const DEFAULT_CANDLE_RANGE = 100;
const DEFAULT_REPLAY_INTERVAL = 1000; // milliseconds
...
module.exports = (client) => class ChartSession {
  ...
  /** @type {PricePeriod[] | null} Cached sorted periods */
  #cachedPeriods = null;

  /** @type {boolean} Flag to indicate if periods have been modified */
  #periodsModified = false;

  /** @return {PricePeriod[]} List of periods values */
  get periods() {
    if (this.#periodsModified || !this.#cachedPeriods) {
      this.#cachedPeriods = Object.values(this.#periods).sort((a, b) => b.time - a.time);
      this.#periodsModified = false;
    }
  ...
  constructor() {
    this.#client.sessions[this.#chartSessionID] = {
      type: 'chart',
      onData: (packet) => {
        if (isDebugEnabled()) console.log('§90§30§106 CHART SESSION §0 DATA', packet);
        ...
        if (['timescale_update', 'du'].includes(packet.type)) {
          ...
          Object.keys(packet.data[1]).forEach((k) => {
            ...
            if (k === '$prices') {
              ...
              periods.s.forEach((p) => {
                // Map TradingView internal bar index -> bar timestamp
                ...
                this.#periods[p.v[0]] = {
                  ...
                  max: p.v[2],
                  min: p.v[3],
                  volume: Math.round(p.v[5] * 100) / 100, // Round to 2 decimal places
                };
              });
              this.#periodsModified = true;
        ...
        if (packet.type === 'symbol_error') {
          const error = new SymbolError(packet.data[2], packet.data[1]);
          this.#handleError(error, `(${packet.data[1]}) Symbol error:`, packet.data[2]);
          return;
        }

        if (packet.type === 'series_error') {
          const error = new SessionError('Series error', packet.data[3]);
          this.#handleError(error, 'Series error:', packet.data[3]);
          return;
        }
        ...
        if (packet.type === 'critical_error') {
          ...
          this.#handleError(error, 'Critical error:', name, description);
        }
      },
    };
    ...
    this.#client.sessions[this.#replaySessionID] = {
      type: 'replay',
      onData: (packet) => {
        if (isDebugEnabled()) console.log('§90§30§106 REPLAY SESSION §0 DATA', packet);
   ...
   */
  setSeries(timeframe = '240', range = DEFAULT_CANDLE_RANGE, reference = null) {
    // Validate inputs
    try {
      validateTimeframe(timeframe, 'timeframe');
      validatePositiveNumber(range, 'range', false);
      validateTimestamp(reference, 'reference');
    } catch (error) {
    ...
    if (!this.#currentSeries) {
      const error = new SessionError('Please set the market before setting series');
      this.#handleError(error);
      throw error;
    }
  ...
  setMarket(symbol, options = {}) {
    ...
    try {
      ...
      validateTimeframe(options.timeframe, 'options.timeframe');
      validatePositiveNumber(options.range, 'options.range', false);
      validateTimestamp(options.to, 'options.to');
      validateAdjustment(options.adjustment, 'options.adjustment');
      validateBoolean(options.backadjustment, 'options.backadjustment');
      validateSession(options.session, 'options.session');
      validateCurrency(options.currency, 'options.currency');
      validateChartType(options.type, 'options.type');
      validateTimestamp(options.replay, 'options.replay');
    } catch (error) {
      this.#handleError(error);
      throw error;
    }
  ...
  fetchMore(number = 1) {
    // Validate inputs
    try {
      validatePositiveNumber(number, 'number', false);
    } catch (error) {
      this.#handleError(error);
      throw error;
    }
  ...
  replayStep(number = 1) {
    return new Promise((cb, reject) => {
      ...
      if (!this.#replayMode) {
        const err = new SessionError('No replay session');
   ...
   */
  replayStart(interval = DEFAULT_REPLAY_INTERVAL) {
    return new Promise((cb, reject) => {
      // Validate inputs
      try {
        validatePositiveNumber(interval, 'interval', false);
      } catch (error) {
        this.#handleError(error);
        reject(error);
        return;
  ...
  onSymbolLoaded(cb) {
    ...
    return () => this.#removeCallback('symbolLoaded', cb);
  }
  ...
  onUpdate(cb) {
    ...
    return () => this.#removeCallback('update', cb);
  }
  ...
  onReplayLoaded(cb) {
    ...
    return () => this.#removeCallback('replayLoaded', cb);
  }
  ...
  onReplayResolution(cb) {
    ...
    return () => this.#removeCallback('replayResolution', cb);
  }
  ...
  onReplayEnd(cb) {
    ...
    return () => this.#removeCallback('replayEnd', cb);
  }
  ...
  onReplayPoint(cb) {
    ...
    return () => this.#removeCallback('replayPoint', cb);
  }
