...
const { IndicatorError, ProtocolError } = require('../errors');
...
module.exports = (chartSession) => class ChartStudy {
  ...
  /** @type {{}[] | null} Cached sorted periods */
  #cachedPeriods = null;
  ...
  constructor(indicator) {
    if (!(indicator instanceof PineIndicator) && !(indicator instanceof BuiltInIndicator)) {
      const error = new IndicatorError(
        'Indicator argument must be an instance of PineIndicator or BuiltInIndicator. Please use \'TradingView.getIndicator(...)\' function.',
        null,
        { receivedType: indicator?.constructor?.name },
      );
      throw error;
    }
    ...
    this.#studyListeners[this.#studID] = async (packet) => {
      if (isDebugEnabled()) console.log('§90§30§105 STUDY §0 DATA', packet);
        ...
        if (data.ns && data.ns.d) {
          ...
          try {
            ...
            const error = new ProtocolError(
              'Study parse error',
              { originalError: e?.message ?? String(e), dataPreview: data.ns.d.slice(0, DATA_PREVIEW_LENGTH) },
            );
            this.#handleError(error, 'Study parse error:', e?.message ?? String(e));
            return;
          }
      ...
      if (packet.type === 'study_error') {
        ...
        this.#handleError(error, packet.data[3], packet.data[4]);
      }
    };
  ...
  onReady(cb) {
    ...
    return () => this.#removeCallback('studyCompleted', cb);
  }
