...
describe('AllErrors', () => {
  const waitForError = (instance: any, next = () => {}) => new Promise<any[]>((resolve) => {
    instance.onError((...error: any[]) => resolve(error));
    next();
  });
  ...
  it('throws an error when an invalid symbol is set', async () => {
    ...
    expect(error).toBeDefined();
    expect(error[0]).toBeInstanceOf(TradingView.SymbolError);
    expect(error[0].message).toBe('invalid symbol');
    expect(error[0].type).toBe('symbol');
    expect(error[1]).toBe('(ser_1) Symbol error:');
    expect(error[2]).toBe('invalid symbol');
  });
  ...
  it('throws an error when an invalid timezome is set', async () => {
    ...
    expect(error).toBeDefined();
    expect(error[0]).toBeInstanceOf(TradingView.ValidationError);
    expect(error[0].message).toContain('timezone is not a valid timezone');
    expect(error[0].type).toBe('validation');
    expect(error[0].field).toBe('timezone');
  });
  ...
  it('throws an error when an invalid timeframe is set', async () => {
    ...
    expect(error).toBeDefined();
    expect(error[0]).toBeInstanceOf(TradingView.ValidationError);
    expect(error[0].message).toContain('options.timeframe must be one of');
    expect(error[0].type).toBe('validation');
    expect(error[0].field).toBe('options.timeframe');
  });
  ...
  it('throws an error when a premium chart type is set without premium', async () => {
    ...
    expect(error).toBeDefined();
    expect(error[0]).toBeInstanceOf(TradingView.SessionError);
    expect(error[0].message).toBe('Series error');
    expect(error[0].type).toBe('session');
    expect(error[1]).toBe('Series error:');
    expect(error[2]).toMatch(/study_not_auth:BarSetRenko@tv-prostudies-\d+/);
  });
  ...
  it('throws an error when series is edited before market is set', async () => {
    ...
    expect(error).toBeDefined();
    expect(error[0]).toBeInstanceOf(TradingView.SessionError);
    expect(error[0].message).toBe('Please set the market before setting series');
    expect(error[0].type).toBe('session');
  });
  ...
  it.skipIf(
    ...
    expect(error).toBeDefined();
    expect(error[0]).toBeInstanceOf(TradingView.IndicatorError);
    expect(error[0].type).toBe('indicator');
    expect(error[1]).toHaveProperty('ctx');
    expect(error[1].ctx.nameInvalidValue).toBe('factor');
